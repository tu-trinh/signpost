...
class Model implements Iterable<Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        for (int i = 0; i < _width; i++) {
            for (int j = 0; j < _height; j++) {
                if (_solution[i][j] == last) {
                    ...
                    Place nextPlace = _solnNumToPlace[seqNum];
                    int dir = Place.dirOf(i, j, nextPlace.x, nextPlace.y);
                    if (seqNum == 1) {
                        _board[i][j] = new Sq(i, j, 1, true, dir, 0);
                    } else {
        ...
        for (int i = 0; i < _width; i++) {
            for (int j = 0; j < _height; j++) {
                ...
                square._successors = allSuccessors(i, j, square.direction());
                square._predecessors = new PlaceList();
                for (int x = 0; x < _width; x++) {
                    for (int y = 0; y < _height; y++) {
                        Sq sq = _board[x][y];
                        if (sq.direction() == Place.dirOf(x, y, i, j)) {
                            square._predecessors.add(Place.pl(x, y));
                        }
                    }
                }
            }
    ...
    Model(Model model) {
        ...
        for (int i = 0; i < _width; i++) {
            for (int j = 0; j < _height; j++) {
                Sq temp = _board[i][j];
                temp._successor = get(model._board[i][j].successor());
                temp._predecessor = get(model._board[i][j].predecessor());
                temp._head = get(model._board[i][j].head());
            }
        }
        _unconnected = last - 1;
    }
                ...
                if (ij.sequenceNum() != 0) {
                    for (int x = 0; x < _width; x++) {
                        for (int y = 0; y < _height; y++) {
                            Sq xy = _board[x][y];
                            if (xy.sequenceNum() != 0) {
                                if (xy.sequenceNum() == ij.sequenceNum() + 1) {
                                    int d = Place.dirOf(ij.x, ij.y, xy.x, xy.y);
                                    if (d == ij.direction()) {
                                        ij.connect(xy);
                                        moved = true;
                                    }
    ...
    void solve() {
        for (int k = 0; k < _solnNumToPlace.length - 1; k++) {
            Place curr = _solnNumToPlace[k];
            Place next = _solnNumToPlace[k + 1];
            this.get(curr).connect(this.get(next));
        }
        _unconnected = 0;
    }
    ...
    private int arrowDirection(int x, int y) {
        ...
        Place otherPlace = _solnNumToPlace[seq0];
        return dirOf(x, y, otherPlace.x, otherPlace.y);
    }

    /** Helper method to find arrowDirection from cell (X, Y). \
     * @return direction as a number */
    public int arrowDir(int x, int y) {
        return arrowDirection(x, y);
    }
    ...
    final class Sq {
        ...
        boolean connectable(Sq s1) {
            boolean correctDir = this.pl.dirOf(s1.pl) == this.direction();
            boolean succ = s1.predecessor() == null && this.successor() == null;
            boolean seq = s1.sequenceNum() != 1 && this.sequenceNum() != size();
            boolean correctSeqNum = true;
            if (this.sequenceNum() != 0 && s1.sequenceNum() != 0) {
                correctSeqNum = this.sequenceNum() == s1.sequenceNum() - 1;
            } else if (this.sequenceNum() == 0 && s1.sequenceNum() == 0) {
                correctSeqNum = this.head() != s1.head();
            }
            return correctDir && succ && seq && correctSeqNum;
        }
        ...
        boolean connect(Sq s1) {
            ...
            if (this.sequenceNum() != 0 && s1.sequenceNum() == 0) {
                releaseGroup(s1Group);
                Sq self = this;
                while (self.successor() != null) {
                    self.successor()._sequenceNum = self.sequenceNum() + 1;
                    self.successor()._head = this.head();
                    self = self.successor();
                }
                s1._head = this.head();
                ...
                Sq self = s1;
                s1._head = this.head();
                while (self.predecessor() != null) {
                    self.predecessor()._sequenceNum = self.sequenceNum() - 1;
                    self.predecessor()._head = this.head();
                ...
                int joinedGroup = joinGroups(thisGroup, s1Group);
                Sq self = this;
                self._group = joinedGroup;
                this.head()._group = joinedGroup;
                while (self.successor() != null) {
                    self.successor()._head = this.head();
                    self.successor()._group = joinedGroup;
                    self = self.successor();
                }
            } else if (this.sequenceNum() != 0 && s1.sequenceNum() != 0) {
                Sq self = this;
        ...
        void disconnect() {
            ...
            if (_sequenceNum == 0) {
                if (this.predecessor() == null && next.successor() == null) {
                    ...
                    next._group = Model.this.newGroup();
                }
            } else {
                if (notFixedPred(this)) {
                    Sq self = this;
                    int newGroup = Model.this.newGroup();
                    while (self != null) {
                        self._sequenceNum = 0;
            ...
            Sq self = next;
            while (self.predecessor() != null) {
                self.predecessor()._head = next;
                self = self.predecessor();
            }
            self = next;
            while (self.successor() != null) {
                self.successor()._head = next;
                self = self.successor();
            }
        }

        /** Checks if predecessors are fixed.
         * TESTER
         * @return bool
         */
        private boolean notFixedPred(Sq tester) {
            boolean notFixed = true;
            Sq testes = tester;
            while (notFixed && testes != null) {
                if (testes.hasFixedNum()) {
                    notFixed = false;
                }
                testes = testes.predecessor();
            }
            return notFixed;
        }

        /** Checks if successors are fixed.
         * TESTER
         * @return bool
         */
        private boolean notFixedSucc(Sq tester) {
            boolean notFixed = true;
            Sq testes = tester;
            while (notFixed && testes != null) {
                if (testes.hasFixedNum()) {
