...
class PuzzleGenerator implements PuzzleSource {
    ...
    static Sq findUniqueSuccessor(Model model, Sq start) {
        ...
        if (start.sequenceNum() != 0) {
            for (Sq square : model) {
                int startN = start.sequenceNum(), sqN = square.sequenceNum();
                if (startN == sqN - 1 && start.connectable(square)) {
                    sequentials++;
                    dongding = square;
                }
            }
    ...
    static Sq findUniquePredecessor(Model model, Sq end) {
        ...
        if (end.sequenceNum() != 0) {
            for (Sq square : model) {
                int sqN = square.sequenceNum(), endN = end.sequenceNum();
                if (sqN != 0 && square.connectable(end) && sqN == endN - 1) {
                    sequentials++;
                    trololo = square;
                }
            }
        ...
        if (sequentials == 1) {
            if (trololo.predecessor() == null && trololo.successor() == null) {
                return trololo;
            }
        } else if (connectables == 1) {
            return tralala;
